    ...
    <script>
        document.getElementById('adminLoginForm').addEventListener('submit', async function(e) {
            ...
            try {
                ...
                // Verificar se é um email administrativo (simulação)
                if (email.includes('admin') || email.includes('@fiveanalysis.com')) {
                    showSuccess('Login administrativo realizado com sucesso! Redirecionando...');
                    setTimeout(() => {
                        window.location.href = 'AdminDashboard.php';
                    }, 1500);
                } else {
                    showError('Acesso negado. Este é um login administrativo.');
                }
            } catch (error) {
