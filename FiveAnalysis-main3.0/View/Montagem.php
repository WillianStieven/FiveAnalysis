...
try {
    // PostgreSQL: usar TRUE ou verificar se ativo é boolean
    $stmt = $conexao->prepare("SELECT p.*, c.nome AS categoria_nome, c.id AS categoria_id, m.nome AS marca_nome 
                                FROM produtos p 
                                LEFT JOIN categorias c ON p.categoria_id = c.id 
                                LEFT JOIN marcas m ON p.marca_id = m.id 
                                WHERE (p.ativo = TRUE OR p.ativo = 't' OR p.ativo = 1)
                                ORDER BY p.data_criacao DESC");
    $stmt->execute();
    $produtos = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    // Debug: verificar se há produtos (apenas em desenvolvimento)
    if (empty($produtos) && isset($_ENV['APP_ENV']) && $_ENV['APP_ENV'] === 'development') {
...
foreach ($produtos as $p) {
  ...
  if (empty($nomeCat)) {
    // Se não tiver categoria, pular ou adicionar a uma categoria padrão
    continue;
  }
  if (!isset($categoriaSlug[$nomeCat])) {
    // Debug: categoria não mapeada
    error_log("Categoria não mapeada: " . $nomeCat);
    continue;
  }
...
// Debug: mostrar quantos produtos foram agrupados por categoria
if (isset($_ENV['APP_ENV']) && $_ENV['APP_ENV'] === 'development') {
    foreach ($bySlug as $slug => $prods) {
        error_log("Categoria $slug: " . count($prods) . " produtos");
    }
}
