...
try {
    // Tentar diferentes formas de verificar ativo (PostgreSQL pode usar boolean ou texto)
    $stmt = $conexao->prepare("SELECT p.*, c.nome AS categoria_nome, c.id AS categoria_id, m.nome AS marca_nome 
                                FROM produtos p 
                                LEFT JOIN categorias c ON p.categoria_id = c.id 
                                LEFT JOIN marcas m ON p.marca_id = m.id 
                                WHERE (p.ativo = TRUE OR p.ativo = 't' OR p.ativo::text = 'true' OR p.ativo = 1)
                                ORDER BY p.data_criacao DESC");
    $stmt->execute();
    $produtos = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    // Se falhar, tentar sem verificação de ativo
    try {
        $stmt = $conexao->prepare("SELECT p.*, c.nome AS categoria_nome, c.id AS categoria_id, m.nome AS marca_nome 
                                    FROM produtos p 
                                    LEFT JOIN categorias c ON p.categoria_id = c.id 
...
foreach ($produtos as $p) {
  ...
  if (!isset($categoriaSlug[$nomeCat])) {
    continue;
  }
...
function extrairAtributosCompat($produto) {
  ...
  if ($categoria === 'Placa Mãe') {
    ...
    if ($ramTipo) {
      $attrs['data-ram-type'] = strtoupper($ramTipo);
      $attrs['data-ramType'] = strtoupper($ramTipo); // Duplicar para compatibilidade
    }
  ...
  if ($categoria === 'Memória RAM') {
    ...
    if ($freq && preg_match('/(\d+)/', $freq, $m)) {
      $attrs['data-ram-speed'] = $m[1];
      $attrs['data-ramSpeed'] = $m[1]; // Duplicar para compatibilidade
    }
  }
  ...
  if ($categoria === 'Placa de Vídeo') {
    ...
    if ($consumo && preg_match('/(\d+)/', (string)$consumo, $m)) {
      $attrs['data-gpu-watts'] = $m[1];
      $attrs['data-gpuWatts'] = $m[1]; // Duplicar para compatibilidade
    }
    // Comprimento da GPU
    $comprimento = $comp['comprimento'] ?? $espec['comprimento'] ?? null;
    if ($comprimento && preg_match('/(\d+)/', (string)$comprimento, $m)) {
      $attrs['data-gpu-length'] = $m[1];
      $attrs['data-gpuLength'] = $m[1]; // Duplicar para compatibilidade
    }
  }

  if ($categoria === 'Fonte') {
    $pot = $espec['potencia'] ?? null; // ex: "650W"
    if ($pot && preg_match('/(\d+)/', (string)$pot, $m)) {
      $attrs['data-psu-watts'] = $m[1];
      $attrs['data-psuWatts'] = $m[1]; // Duplicar para compatibilidade
    }
  }
  ...
  if ($categoria === 'Armazenamento') {
    ...
    if (strpos($interface, 'PCIE') !== false || strpos($interface, 'NVME') !== false || strpos($interface, 'M.2') !== false) {
      $attrs['data-storage-type'] = 'NVME';
      $attrs['data-storageType'] = 'NVME'; // Duplicar para compatibilidade
    } elseif (strpos($interface, 'SATA') !== false) {
      $attrs['data-storage-type'] = 'SATA';
      $attrs['data-storageType'] = 'SATA'; // Duplicar para compatibilidade
    }
    if ($formato) {
      $attrs['data-form-factor'] = $formato;
      $attrs['data-formFactor'] = $formato; // Duplicar para compatibilidade
    }
  }

  if ($categoria === 'Gabinete') {
    $alturaGpu = $comp['altura_max_gpu'] ?? $comp['comprimento_max_gpu'] ?? $espec['comprimento_max_gpu'] ?? null; // ex: "360mm"
    if ($alturaGpu && preg_match('/(\d+)/', (string)$alturaGpu, $m)) {
      $attrs['data-gpu-max-length'] = $m[1];
      $attrs['data-gpuMaxLength'] = $m[1]; // Duplicar para compatibilidade
    }
  }
...
<?php 
if (!empty($bySlug['processador'])): 
    foreach ($bySlug['processador'] as $p): 
        $attrs = extrairAtributosCompat($p); 
?>
      <div class="product-card" id="product-<?= $p['id'] ?>" style="display: block;"<?= attr('data-brand', $p['marca_nome']) ?><?= isset($attrs['data-socket']) ? attr('data-socket', $attrs['data-socket']) : '' ?>>
...
?>
      <p style="grid-column: 1/-1; text-align:center; color: var(--text-muted);">Sem CPUs ativas. Total de produtos no banco: <?= count($produtos) ?></p>
<?php endif; ?>
    </div>
...
<?php endif; ?>
  ...
  <script src="../Assets/js/montar-computador.js"></script>
  <script>
